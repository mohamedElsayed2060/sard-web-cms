...
export default function AboutSardAwards({ awards = [], bgImage }) {
  const activeAwards = useMemo(
    () => (awards || []).filter((a) => a?.isActive !== false),
    [awards]
  );
  ...
  const onPick = (clickedIdx) => {
    ...
    // اخفي الكونتنت فورًا (زي ما عندك)
    ...
    timerRef.current = setTimeout(() => {
      ...
      // ✅ رجّع label بتاع الزرار اللي قفل بعد ما يستقر
  ...
  const ordered = order
    .map((idx) => ({ idx, item: activeAwards[idx] }))
    .filter((x) => x.item);
        ...
        <div className="bg-black px-3 pb-5 max-w-[1490px] mx-auto">
          ...
          >
            ...
            <div className="hidden md:block relative">
              {/* ✅ مفيش height ثابت: خليها min-height بس */}
                  ...
                  return (
                    <motion.button
                      ...
                      className={[
                        ...
                        isActive
                          ? "flex-[1_1_0%] bg-transparent border-0"
                    ...
                    >
                      {isActive ? (
                        <motion.div
                          layout
                          className="h-full w-full text-left p-8"
                        >
                          {/* ✅ العنوان والكونتنت يختفوا فورًا لما showContent=false */}
            ...
            <div className="md:hidden">
              {/* ✅ مفيش height ثابت، نخليها min-height وتكبر حسب الكونتنت */}
              <div className="relative">
                ...
                {/* Right stacked buttons (في مكانهم يمين + فوق بعض) */}
                <div className="absolute top-4 right-4 w-[50px]">
                  {ordered
                    .slice(1, 1 + MOBILE_TABS_COUNT)
                    .map(({ item, idx }, i) => {
                      const y = i * 18; // كل واحدة نازلة حتة صغيرة
                      return (
                        <motion.button
                          key={item?.id || idx}
                          layout
